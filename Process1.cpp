...
DWORD WINAPI Delete(void*)
{
	...
	{
		cout << "Process1 is unable to open mutex: " << GetLastError() << endl;
		exit(1);
	}
	...
	while (true)
	{
		...
		for (int i = 0; i < r; i++)
		{
			if (vector.size() > 0)
			{
				...
				std::cout << "Group_1      --1      " << vector.size() << std::endl;
				ReleaseMutex(mutex);
			}
			else
			{
...
#pragma endregion
	...
	HANDLE arr[] = { Th3,Th2,Th3 };
	WaitForMultipleObjects(3, arr, FALSE, INFINITE);


	// Kill all threads
	TerminateThread(Th1, 1);
	TerminateThread(Th2, 1);
	TerminateThread(Th3, 1);

	// Group_1 will lost if one of threads ends
	WaitForSingleObject(mutex, INFINITE);
	std::cout << "Group_1 Lost :(\n";
	ReleaseMutex(mutex);
